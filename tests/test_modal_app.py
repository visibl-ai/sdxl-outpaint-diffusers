...
from modal_app import Inference, web_app
...
class TestModalApp(unittest.TestCase):
    ...
    def test_inference_endpoint_single_image(self, mock_inference):
        ...
        mock_instance.run.remote.aio = AsyncMock(
            return_value="http://example.com/result.png"
        )
    ...
    def test_inference_endpoint_invalid_input(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json(), {"error": "Either input or batch must be provided"}
        )
    ...
    def test_inference_upload_url(self, mock_inference):
        ...
        mock_instance = AsyncMock()
        mock_instance.run.remote.aio = AsyncMock(
            return_value="http://example.com/uploaded.png"
        )
        mock_inference.return_value = mock_instance
